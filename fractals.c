...
/*   Updated: 2023/12/05 19:58:47 by vshchuki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_point_mandelbrot(t_state *state, double x0, double y0)
{
	...
	y = 0;
	iter = 0;
	while (x * x + y * y <= 2 * 2 && iter < state->max_iter)
	{
		x_temp = x * x - y * y + x0;
		y = 2 * x * y + y0;
		x = x_temp;
...
int	ft_check_point_julia(t_state *state, double x, double y)
{
	...
	iter = 1;
	while (x * x + y * y <= r2 && iter < state->max_iter)
	{
		x_temp = x * x - y * y;
		y = 2 * x * y + state->cy;
		x = x_temp + state->cx;
...
int	ft_check_point_tricorn(t_state *state, double x, double y)
{
	...
	(void)state;
	iter = 1;
	while (x * x + y * y <= 4 && iter < state->max_iter)
	{
		x_temp = x * x - y * y + x;
		y = -2 * x * y + y;
		x = x_temp;
...
int	ft_check_point_baldelbrot(t_state *state, double x, double y)
{
	...
	while (magnitude < 1 && iter < state->max_iter)
	{
		if (x > 1)
			x = 2 - x;
		else if (x < -1)
